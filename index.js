...
((calcium) => {
  ...
  class Engine {
    ...
    debugPrint(expr, callback) {
      ...
      const result =
        makeDescription(exprObj.debugEvaluate(this.environment));
      callback(result);
    }
    ...
    step() {
      ...
      while (keyword === calcium.keyword.COMMENT
  ...
  class ClassBuilder {
    ...
    inherit(superclass) {
      if (superclass instanceof ClassBuilder) {
        ...
        throw new SuperclassNotValidError(
          'superclass must be a ClassBuilder');
      }
    }
  }
    ...
    * buildAll() {
      for (let module of this.modules) {
        ...
        const moduleObj = new Module(
          module.name, classes, module.functions);
        yield moduleObj;
      }
    }
  }
  ...
  class Parser {
    constructor() {
      ...
      _table[calcium.keyword.ASSIGNMENT] = lineArray => {
        const lhs = this.parseReference(
          lineArray[calcium.index.ASSIGNMENT_LHS]);
        const rhs = this.parseExpression(
          lineArray[calcium.index.ASSIGNMENT_RHS]);
        return new Assignment(lhs, rhs);
      };

      _table[calcium.keyword.BREAK] = () => new Break();

      _table[calcium.keyword.CALL] = lineArray => {
        ...
        const args = this.parseArgs(
          lineArray, calcium.index.CALL_ARGS);
        return new Call(lhs, calledFuncRef, args);
      };

      _table[calcium.keyword.CLASS_DEF] = lineArray => {
        const className =
          lineArray[calcium.index.CLASS_DEF_CLASS_NAME];
        const superclassName =
          lineArray[calcium.index.CLASS_DEF_SUPERCLASS_NAME];
        return new ClassDef(className, superclassName);
      };

      _table[calcium.keyword.COMMENT] = lineArray => {
        const options = lineArray[calcium.index.OPTIONS];
        return new Comment(options);
      };

      _table[calcium.keyword.COMPOUND_ADDITION] = lineArray => {
        return _makeCompoundAssignment(
          calcium.keyword.binaryOperator.ADDITION, lineArray);
      };

      _table[calcium.keyword.COMPOUND_SUBTRACTION] = lineArray => {
        return _makeCompoundAssignment(
          calcium.keyword.binaryOperator.SUBTRACTION, lineArray);
      };

      _table[calcium.keyword.COMPOUND_MULTIPLICATION] = lineArray => {
        return _makeCompoundAssignment(
          calcium.keyword.binaryOperator.MULTIPLICATION, lineArray);
      };

      _table[calcium.keyword.CONTINUE] = () => new Continue();

      _table[calcium.keyword.ELIF] = lineArray => {
        const condition = this.parseExpression(
          lineArray[calcium.index.CONDITION]);
        return new Elif(condition);
      };

      _table[calcium.keyword.ELSE] = () => new Else();

      _table[calcium.keyword.END_OF_CODE] =
        () => new EndOfCode();

      _table[calcium.keyword.EXCEPT] = lineArray => {
      ...
      _table[calcium.keyword.FOR_EACH] = lineArray => {
        const elementName =
          lineArray[calcium.index.FOR_EACH_ELEMENT_NAME];
        const iterable = this.parseExpression(
          lineArray[calcium.index.FOR_EACH_ITERABLE]);
        return new ForEach(elementName, iterable);
      };

      _table[calcium.keyword.FOR_RANGE] = lineArray => {
        const variableName = 
          lineArray[calcium.index.FOR_RANGE_VARIABLE_NAME];
      ...
      _table[calcium.keyword.FUNC_DEF] = lineArray => {
      ...
      _table[calcium.keyword.IF] = lineArray => {
        const condition = this.parseExpression(
          lineArray[calcium.index.CONDITION]);
        return new If(condition);
      };

      _table[calcium.keyword.IFS] = () => new Ifs();

      _table[calcium.keyword.IMPORT] = lineArray => {
        const moduleName =
          lineArray[calcium.index.IMPORT_MODULE_NAME];
        return new Import(moduleName);
      };

      _table[calcium.keyword.PASS] = () => new Pass();

      _table[calcium.keyword.RAISE] = lineArray => {
        const exceptionName =
          lineArray[calcium.index.RAISE_EXCEPTION];
        const args = this.parseArgs(
          lineArray, calcium.index.RAISE_ARGS);
        return new Raise(exceptionName, args);
      };

      _table[calcium.keyword.RETURN] = lineArray => {
      ...
      _table[calcium.keyword.TRY] = () => new Try();

      _table[calcium.keyword.WHILE] = lineArray => {
        const condition = this.parseExpression(
          lineArray[calcium.index.CONDITION]);
      ...
      const _self = this;
      function _makeCompoundAssignment(keyword, lineArray) {
        const lhs = _self.parseReference(
          lineArray[calcium.index.ASSIGNMENT_LHS]);
      ...
      this.binaryOperators =
        Object.values(calcium.keyword.binaryOperator);
      this.unaryOperators =
        Object.values(calcium.keyword.unaryOperator);
    }
    parse(lineArray) {
    ...
    parseReference(arrayObj) {
      ...
      if (keyword === calcium.keyword.VARIABLE) {
        ...
        const propertyNames = [];
        for (let i = calcium.index.ATTRIBUTE_PROPERTY_NAMES;
          i < arrayObj.length; ++i) {
  ...
  class Attribute {
    ...
    assign(value, env) {
      ...
      target.setAttribute(
        this.propertyNames[this.propertyNames.length - 1], value);
    }
    debugEvaluate(env) {
      const instance = env.debugLookUp(this.objName);
      let target = instance;
      for (let prop of this.propertyNames) {
        if (target instanceof ClassObj
    ...
    // get a built-in method of built-in types
    getAttribute(obj, name) {
      if (obj instanceof Array) {
        ...
        if (name === 'find' || name in methodNames['find']) {
          ...
          || name in methodNames['replace']) {
          return new BuiltinFuncObj(name, function (args, env) {
            let fromStr = env.evaluate(args[0]);
            let toStr = env.evaluate(args[1]);
            return obj.replace(new RegExp(fromStr, 'g'),
              toStr);
          });
        }
      } else if ((name === 'keys' || name in methodNames['keys'])
  ...
  class BinaryOperation {
    constructor(operator, leftOperand, rightOperand) {
      ...
      table[calcium.keyword.binaryOperator.ADDITION] =
        (leftOperand, rightOperand) => leftOperand + rightOperand;

      table[calcium.keyword.binaryOperator.SUBTRACTION] =
        (leftOperand, rightOperand) => leftOperand - rightOperand;

      table[calcium.keyword.binaryOperator.MULTIPLICATION] =
      ...
      table[calcium.keyword.binaryOperator.EXPONENTIATION] =
        (leftOperand, rightOperand) =>
          Math.pow(leftOperand, rightOperand);

      table[calcium.keyword.binaryOperator.DIVISION] =
        (leftOperand, rightOperand) => (leftOperand / rightOperand);
      
      table[calcium.keyword.binaryOperator.FLOOR_DIVISION] =
        (leftOperand, rightOperand) =>
          Math.floor(leftOperand / rightOperand);

      table[calcium.keyword.binaryOperator.REMAINDER] =
        (leftOperand, rightOperand) => leftOperand % rightOperand;

      table[calcium.keyword.binaryOperator.EQUAL] =
        (leftOperand, rightOperand) => {
      ...
      table[calcium.keyword.binaryOperator.NOT_EQUAL] =
        (leftOperand, rightOperand) =>
          !table[calcium.keyword.binaryOperator.EQUAL](
            leftOperand, rightOperand);

      table[calcium.keyword.binaryOperator.LESS_THAN] =
        (leftOperand, rightOperand) => leftOperand < rightOperand;

      table[calcium.keyword.binaryOperator.LESS_THAN_OR_EQUAL] =
        (leftOperand, rightOperand) => leftOperand <= rightOperand;

      table[calcium.keyword.binaryOperator.GREATER_THAN] =
        (leftOperand, rightOperand) => leftOperand > rightOperand;

      table[calcium.keyword.binaryOperator.GREATER_THAN_OR_EQUAL] =
        (leftOperand, rightOperand) => leftOperand >= rightOperand;

      table[calcium.keyword.binaryOperator.AND] =
        (leftOperand, rightOperand) => leftOperand && rightOperand;

      table[calcium.keyword.binaryOperator.OR] =
        (leftOperand, rightOperand) => leftOperand || rightOperand;

      table[calcium.keyword.binaryOperator.IS] =
        (leftOperand, rightOperand) => leftOperand === rightOperand;

      table[calcium.keyword.binaryOperator.IS_NOT] =
        (leftOperand, rightOperand) => leftOperand !== rightOperand;

      table[calcium.keyword.binaryOperator.IN] =
        (leftOperand, rightOperand) => {
      ...
      table[calcium.keyword.binaryOperator.NOT_IN] =
        (leftOperand, rightOperand) =>
        !table[calcium.keyword.binaryOperator.IN](
          leftOperand, rightOperand);

      table[calcium.keyword.binaryOperator.BITWISE_AND] =
        (leftOperand, rightOperand) => leftOperand & rightOperand;

      table[calcium.keyword.binaryOperator.BITWISE_OR] =
        (leftOperand, rightOperand) => leftOperand | rightOperand;

      table[calcium.keyword.binaryOperator.BITWISE_XOR] =
        (leftOperand, rightOperand) => leftOperand ^ rightOperand;

      table[calcium.keyword.binaryOperator.LEFT_SHIFT] =
        (leftOperand, rightOperand) => leftOperand << rightOperand;

      table[calcium.keyword.binaryOperator.RIGHT_SHIFT] =
        (leftOperand, rightOperand) => leftOperand >> rightOperand;
    ...
    get description() {
      ...
      return wrapDescriptionByParen(leftOperand) + ` ${this.operator} ` +
  ...
  class Environment {
    constructor(code, parser) {
      ...
      _builtinFunctions[builtin.name.ISINSTANCE] = new BuiltinFuncObj(
        builtin.name.ISINSTANCE, (args, env) => {
      ...
      _builtinFunctions[builtin.name.PRINT] = new BuiltinFuncObj(
        builtin.name.PRINT, (args, env) => {
          const descriptions = args.map(
            v => env.evaluate(v)
          ).map(
            v => makeDescription(v)
          );
      ...
      _builtinFunctions[builtin.name.STR] = new BuiltinFuncObj(
        builtin.name.STR, (args, env) => {
          return makeDescription(env.evaluate(args[0]));
        }
      );

      _builtinFunctions[builtin.name.SUPER] = new BuiltinFuncObj(
        builtin.name.SUPER, (args, env) => {
          const classObj = env.evaluate(args[0]);
    ...
    debugEvaluate(expr) {
      ...
      } else if (expr instanceof ClassObj
        ...
        if (value !== undefined) {
          ...
          if (currentScope === null) {
            ...
            throw new NameNotFoundError(
              `${name} not found`, this.address.index, name);
          } else {
            continue;
          }
        }
    ...
    endBlock() {
      try {
        ...
        block.end(this);
        switch (block.kind) {
          case calcium.block.IFS:
          case calcium.block.CLASS_DEF:
          case calcium.block.TRY:
          case calcium.block.EXCEPT:
            return false;
          case calcium.block.IF_ELIF_ELSE:
          case calcium.block.CALL:
            return true;
          case calcium.block.WHILE:
          case calcium.block.FOR_RANGE:
          case calcium.block.FOR_EACH:
    ...
    _findExcept() {
      ...
      for (;;) {
        ...
        if (indent < this.address.indent) {
          ...
          if (nextLine[calcium.index.KEYWORD] ===
            calcium.keyword.EXCEPT
            && indent === this.address.indent) {
            this.jumpTo(new Address(this.address.indent,
              nextIndex));
    ...
    raiseException(exceptionName, args=[]) {
      ...
      for (;;) {
        ...
        try {
          block = this.popBlock();
          if (block.kind !== calcium.block.TRY) {
            if (block.kind === calcium.block.CALL) {
              this.popCallStack();
            }
            continue;
          }
        ...
        for (;;) {
          ...
          const exceptionClassToCatch =
            this.lookUp(exceptCommand.typeName);
          const shouldBeCaught =
            this.builtinFunctions[builtin.name.ISINSTANCE]
            .body([raisedExceptionObj,
              exceptionClassToCatch], this);
          if (shouldBeCaught) {
            this.exception = raisedExceptionObj;
            return;
          } else {
  ...
  class Subscript {
    ...
    debugEvaluate(env) {
      ...
      if (value !== undefined) {
        ...
        throw new ValueNotFoundError(
          `${subscript} not found`, env.address.index, subscript);
      }
    }
    evaluate(env) {
      const obj = this.lookUp(env);
    ...
    get description() {
      return this.objRef.description +
        `[${makeDescription(this.indexExpr)}]`;
    }
  }
  ...
  class Super {
    ...
    getAttribute(name) {
      ...
      for (;;) {
        if (!currentClassObj) {
          ...
          throw new SuperCallFailedError(
            '', this.classObj.name, name);
        }
    ...
    get description() {
      return `<super of ${this.classObj.name} on ` +
  ...
  class UnaryOperation {
    constructor(operator, operand) {
      ...
      table[calcium.keyword.unaryOperator.NOT] = operand => !operand;
      table[calcium.keyword.unaryOperator.NEGATIVE] = operand => -operand;
      table[calcium.keyword.unaryOperator.BITWISE_NOT] = operand => ~operand;
    ...
    get description() {
      if (this.operator === calcium.keyword.unaryOperator.NEGATIVE) {
        return '-' + wrapDescriptionByParen(this.operand);
      } else if (this.operator === calcium.keyword.unaryOperator.NOT) {
      ...
      for (let elem of obj) {
        if (elem === obj) {
          ...
          if (typeof elem === 'string'
            ...
            desc.push(makeDescription(elem));
          }
        }
      }
      return `[${desc.join(', ')}]`;
      ...
      for (let key in obj) {
        ...
        if (value === obj) {
          ...
          if (typeof value === 'string'
            ...
            kvStr = kvStr + makeDescription(value);
          }
        }
        desc.push(kvStr);
      }
  ...
  function registerExceptionClass(jsErrorClass, env) {
    ...
    const exceptionClassObj =
      new ClassObj(jsErrorObj.name, builtin.type.exception, {});
  ...
  // enum which represents the kind of block
  calcium.block = {
    IFS: 0,
    IF_ELIF_ELSE: 1,
    FOR_RANGE: 2,
  ...
  calcium.builtin = builtin; // publish
  ...
  builtin.type.builtin_function_or_method = new ClassObj(
    'builtin_function_or_method', builtin.type.object
  );
  ...
  builtin.type.function = new ClassObj(
    'function', builtin.type.object
  );

  builtin.type.instance_method = new ClassObj(
    'instancemethod', builtin.type.object
  );

  builtin.type.module = new ClassObj(
    'module', builtin.type.object
  );

  builtin.type.super = new ClassObj(
    'super', builtin.type.object
  );
  ...
  class Break {
    ...
    execute(env) {
      for (;;) {
        ...
        if (block.kind === calcium.block.IFS
          || block.kind === calcium.block.IF_ELIF_ELSE
          || block.kind === calcium.block.TRY
          || block.kind === calcium.block.EXCEPT) {
          env.shiftIndent(-1);
          continue;
        } else if (block.kind === calcium.block.WHILE
          || block.kind === calcium.block.FOR_RANGE
          || block.kind === calcium.block.FOR_EACH) {
          env.shiftIndent(-1);
          break;
        } else { // CALL, CLASS_DEF, etc.
  ...
  class Call {
    ...
    execute(env) {
      ...
      if (calledObj instanceof FuncObj
        || calledObj instanceof MethodObj) {
        ...
        const returnedValue = calledObj.funcObj.body(
          this.args, env);
        if (this.lhs !== null) {
          this.lhs.assign(returnedValue, env);
        }
        ...
        try {
          ...
          for (let alternativeInitName in
              methodNames[builtin.name.INIT]) {
    ...
    execute(env) {
      ...
      const end = env => {
        ...
        const classObj = new ClassObj(
          className, superclass, attributes);
        env.register(classObj.name, classObj);
        env.shiftIndent(-1);
      };
      const block = new Block(
        calcium.block.CLASS_DEF, env.address, begin, end);
  ...
  class Continue {
    ...
    execute(env) {
      for (;;) {
        let block = env.popBlock();
        if (block.kind === calcium.block.WHILE
          || block.kind === calcium.block.FOR_RANGE
          || block.kind === calcium.block.FOR_EACH) {
          env.beginBlock(block);
          break;
        } else if (block.kind === calcium.block.IFS
          || block.kind === calcium.block.IF_ELIF_ELSE
          || block.kind === calcium.block.TRY
          || block.kind === calcium.block.EXCEPT) {
          env.shiftIndent(-1);
  ...
  class Except {
    ...
    execute(env) {
      ...
      const block = new Block(
        calcium.block.EXCEPT, env.address, begin, end);
      env.beginBlock(block);
    }
  }
    ...
    execute(env) {
      ...
      if (iterableObj instanceof Array
        ...
        begin = env => {
          const nextIndex = loopCounter.next();
          if (nextIndex !== null) {
            env.register(elementName, 
              iterableObj[keys[nextIndex]]);
            return true;
          } else {
            return false;
          }
      ...
      const end = () => { /* Do nothing */ };
      const block = new Block(
        calcium.block.FOR_EACH, env.address, begin, end);
      env.beginBlock(block);
    }
  }
  ...
  class ForRange {
    ...
    execute(env) {
      ...
      const end = () => { };
      const block = new Block(
        calcium.block.FOR_RANGE, env.address, begin, end);
      env.beginBlock(block);
    }
  }
  ...
  class FuncDef {
    ...
    execute(env) {
      ...
      const funcObj = new FuncObj(
        this.name, this.params, nestingScope, definedAddress);
  ...
  class Ifs {
    ...
    execute(env) {
      ...
      const end = env => env.shiftIndent(-1);
      const block = new Block(
        calcium.block.IFS, env.address, begin, end);
      env.beginBlock(block);
    }
  }
  ...
  class Try {
    ...
    execute(env) {
      ...
      const end = env => env.shiftIndent(-1);
      const block = new Block(
        calcium.block.TRY, env.address, begin, end);
      env.beginBlock(block);
    }
  }
  ...
  class While {
    ...
    execute(env) {
      ...
      const end = () => { };
      const block = new Block(
        calcium.block.WHILE, env.address, begin, end);
      env.beginBlock(block);
    }
  }
  ...
  function executeConditionalBlock(env) {
    ...
    const block = new Block(calcium.block.IF_ELIF_ELSE,
      env.address, begin, end);
    env.beginBlock(block);
  }
  ...
  function invoke(calledObj, args, lhs, env) {
    ...
    const newContext = new FuncScope(
      funcObj.nestingScope, argsDict);
    ...
    const block = new Block(calcium.block.CALL,
